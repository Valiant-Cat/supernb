...
8. Carry depth through all phases:
   ...
   - design should use the local `impeccable` router and concrete skills like `frontend-design`, `critique`, `polish`, and `audit` to define system-level visual rules, key surfaces, interaction quality, trust, conversion, retention, and responsive behavior
   - implementation and release artifacts should be concrete enough to certify

Use plain local skill names for this design stack. Do not invent namespaced local calls such as `superpowers:ui-ux-upgrade`.
...
2. Run `prompt-closeout`.
3. Let `prompt-closeout` import the report and apply the correct certification behavior for the active phase.
...
1. Start the Ralph Loop contract generated by `prompt-bootstrap`.
...
4. Use `prompt-closeout` so the promise line is only emitted after successful closeout.
5. If the stop-hook is unavailable, downgrade the batch to `needs-follow-up` instead of claiming clean completion.
