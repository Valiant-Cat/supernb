...
def write_reassessment_template(target: Path, spec: dict[str, Any], selected_phase: str) -> None:
    ...
    lines = [
        ...
        "",
        "- What concrete product, UX, architecture, reliability, or growth gaps are visible right now?",
        "- Which current `.supernb` artifacts are stale, too thin, or out of sync with the real workspace state?",
        "",
        "## Key Gaps",
        "",
        "- Product or user-value gap:",
        "- UX or design gap:",
        "- Engineering, reliability, or trust gap:",
        "- Localization, analytics, or release gap:",
        "",
        "## Artifact Drift",
        "",
        "- Earliest stale artifact or phase:",
        "- Why that artifact is no longer trustworthy enough to keep shipping from:",
        "- Which other artifacts must be upgraded in the same reopen window:",
        "",
        "## Reopen Decision",
        "",
        ...
        "",
        "- Research or PRD upgrades required:",
        "- Design upgrades required:",
        "- Planning, delivery, or release upgrades required:",
        "",
        "## Execution Rule",
        "",
        "- Replace the blank bullets above with concrete facts before changing `- Status:` to `completed`.",
...
def write_loop_prompt(
    ...
    lines = [
        ...
        "- Template-grade reassessment notes or prompt reports will be rejected during managed closeout. Replace placeholders with concrete batch evidence first.",
...
def write_prompt_session(
    ...
    lines = [
        ...
        "10. Do not leave the reassessment or prompt report in template form. Managed closeout rejects metadata-only or placeholder-heavy submissions.",
        "",
        "## Required Files",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Template-grade reassessment notes or prompt reports are rejected before import, even if the metadata fields were flipped to `completed`.",
